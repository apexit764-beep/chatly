...
interface SocialAuthButtonsProps {
  onPick: (provider: AuthProvider) => void | Promise<void>;
  disabled?: boolean;
  /** "تسجيل الدخول" vs "المتابعة" — the verb differs between the two screens. */
  verb?: string;
}

/** Google's mark. Inlined rather than fetched — an auth screen should not wait on a CDN. */
...
/** Apple's mark is monochrome and must invert with the theme. */
...
export function SocialAuthButtons({
  ...
  const run = async (provider: AuthProvider): Promise<void> => {
  ...
  const base = cn(
    'h-12 flex-1 rounded-xl border border-border-light dark:border-border-dark',
  ...
  const spinner = <span className="h-4 w-4 rounded-full border-2 border-current border-t-transparent animate-spin" />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => run('google')} disabled={disabled || !!busy} className={base}>
          {busy === 'google' ? spinner : <GoogleMark />}
          <span>{verb} بحساب Google</span>
        </button>
        <button type="button" onClick={() => run('apple')} disabled={disabled || !!busy} className={base}>
          {busy === 'apple' ? spinner : <AppleMark />}
