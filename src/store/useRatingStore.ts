...
export interface Rating {
  ...
  channelName: string;
  /**
   * Snapshot of the sending account's rating settings, taken when the link was
   * issued — the survey must keep the shape the customer was promised even if
   * the account's settings change before they open it.
   */
...
interface RatingState {
  ratings: Rating[];
  generateToken: (
    data: Omit<Rating, 'token' | 'createdAt' | 'expiresAt' | 'submittedAt' | 'ratingConversation' | 'ratingAgent' | 'satisfaction' | 'comment'>,
    expireDays?: number,
  ) => string;
...
const MOCK_RATINGS: Rating[] = [
  {
    ...
    agentId: 'a1', agentName: 'سالم الرواحي', channelType: 'whatsapp', channelName: 'واتساب الرئيسي', askAgentRating: true,
  ...
  },
  {
    ...
    agentId: 'a2', agentName: 'مريم الكندي', channelType: 'whatsapp', channelName: 'واتساب الرئيسي', askAgentRating: true,
  ...
  },
  {
    ...
    agentId: 'a1', agentName: 'سالم الرواحي', channelType: 'email', channelName: 'البريد الرسمي', askAgentRating: true,
  ...
  },
  {
    ...
    agentId: 'a3', agentName: 'عبدالله الراشدي', channelType: 'whatsapp', channelName: 'واتساب الرئيسي', askAgentRating: true,
  ...
  },
  {
    ...
    agentId: 'a3', agentName: 'عبدالله الراشدي', channelType: 'email', channelName: 'البريد الرسمي', askAgentRating: true,
...
function read(): Rating[] {
  ...
  try {
    ...
    if (parsed.length === 0) return MOCK_RATINGS;
    // askAgentRating was added later — links issued before it default to asking.
    return parsed.map((r) => ({ ...r, askAgentRating: r.askAgentRating ?? true }));
  } catch {
    return MOCK_RATINGS;
  }
}
...
export const useRatingStore = create<RatingState>((set, get) => ({
  ratings: read(),
  generateToken: (data, expireDays = DEFAULT_EXPIRY_DAYS) => {
    const token = makeToken();
    const now = new Date();
    const expires = new Date(now.getTime() + expireDays * 24 * 60 * 60 * 1000);
    const rating: Rating = {
      ...data,
      token,
